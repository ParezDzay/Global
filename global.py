...
def push_to_github(file_path, commit_message):
    try:
        ...
        pass  # ignore GitHub errors for UX
        ...
        for d in sorted(upcoming["Date"].dt.date.unique()):
            ...
            with st.expander(d.strftime("📅 %A, %d %B %Y")):
                ...
                for i, row in sub.iterrows():
                    ...
                    if c3.button("Delete", key=f"del_{d}_{i}"):
                        modify_booking(row, "Delete"); safe_rerun()

# ===== Tab 2 — Archive (all Confirmed) =====
with tab_archive:
    df = load_bookings()
    confirmed = df[df["Status"] == "Confirmed"]   # immediate archive
    st.subheader("📂 Operation Archive")
    if confirmed.empty:
        st.info("No confirmed records yet.")
    else:
        table = confirmed.copy()
        ...
        st.markdown(
            table.to_html(escape=False,
                          columns=["Date", "Doctor", "Surgery", "Hour", "Room"],
                          index_names=False),
...
# 30-min slots 10:00–22:00 (no 22:30)
