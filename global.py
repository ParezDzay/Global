...
# -------------------------------------------------------------
# Tabs — Add and View Bookings
# -------------------------------------------------------------
tab1, tab2 = st.tabs(["➕ Add Booking", "📋 View Saved Bookings"])

bookings = load_bookings()

with tab1:
    st.subheader("➕ Register a New Operation")
    ...
    sel_hour_str = st.selectbox("Hour", slot_display)
    sel_hour = datetime.strptime(sel_hour_str, "%H:%M").time()

    doctor_name = st.text_input("Doctor Name")
    surgery_choice = st.selectbox("Surgery Type", SURGERY_TYPES)
    ...
    if st.button("💾 Save Booking"):
        if not doctor_name:
            ...
            st.success(f"✅ Surgery #{next_id} saved and pushed to GitHub!")
            safe_rerun()
